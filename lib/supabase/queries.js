...
/**
 * All rows of the k-anonymised field_stats view (0009 hardening). Two
 * mutually-exclusive sample-size columns now exist — `sample_size` (exact,
 * suppressed fields only) and `sample_size_band` (banded, unsuppressed
 * fields only) — see lib/explore/sampleSize.js, the single place that
...
export async function fetchFieldStats(supabase) {
  const { data, error } = await supabase
    .from('field_stats')
    .select(
      'field_of_study, sample_size, sample_size_band, avg_satisfaction, pct_dissatisfied, common_preu, suppressed',
    );
...
/**
 * All rows of the k-anonymised field_detail_stats view (0008, hardened by
 * 0010_field_detail_stats_hardening.sql) — satisfaction distribution,
 * pre-U route breakdown and SPM stream breakdown per field, for
 * /explore/[field]. Suppressed fields (n<10) come back with
 ...
 *
 * Since 0010: `sample_size`/`sample_size_band` are republished straight
 * from field_stats_cache (the same row fetchFieldStats reads), so they
 * always agree with field_stats — never rely on this row's own
 * sample_size for anything field_stats didn't already say. The three
 * distribution objects now hold ROUNDED PERCENTAGES (nearest 5, 5 floor
 * for any present category), never exact counts — see
 * 0010_field_detail_stats_hardening.sql's header for why an exact
 * histogram was a worse version of the leak 0009 closed on field_stats.
 */
export async function fetchFieldDetailStats(supabase) {
  const { data, error } = await supabase
    .from('field_detail_stats')
    .select(
      'field_of_study, sample_size, sample_size_band, suppressed, satisfaction_distribution, preu_distribution, stream_distribution',
    );
