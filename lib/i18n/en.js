...
const en = {
  ...
  privacy: {
    ...
    sections: [
      ...
      },
      {
        ...
        body: 'The delete control on your account page removes your answers, your results and your account. It is immediate and it is not recoverable — we keep no shadow copy. If you contributed your own outcome through the contribute page, that submission is anonymous and unlinked, so it stays in the dataset.',
      },
      {
  ...
  contribute: {
    ...
    body: 'Tell us what you actually chose, and the next student gets a better answer. It’s the same ten questions ARAH asks, plus what you studied, why, and how it went.',
    ...
    signInPrompt: {
      ...
      body: 'We ask you to sign in so a person can review this against the rest of the data before it counts — the same reason you sign in before answering the questions.',
      cta: 'Sign in',
    },
  ...
  api: {
    ...
    // /api/admin/contributions (Plan 5, Task 5) — the moderation mutation
    // route. is_admin is re-checked here regardless of what the UI showed;
    // these are the messages a caller sees when that check fails.
    admin: {
  ...
  landing: {
    ...
    proof: {
      ...
      // Measured by ml/measure_paths.py — a committed, re-runnable script, so
      // these can be checked rather than taken on trust. Both paths use the
      // same repeated stratified CV (5 folds x 5 repeats) as training.
      //
      // Updated 2026-07-30 when the ensemble moved to equal voting weights.
      // Part of the move from 69.1 is the weight change and part is a more
      // faithful measurement: re-measuring the OLD model with this script
      // gives 70.0 / 63.9, not the 69.1 / 62.8 published before it existed.
      // The weight change itself is +1.5pp on the direct path and neutral on
      // the marginalised one.
      withRoute: { label: 'With a stated pre-U route', value: '71.5%' },
      withoutRoute: { label: 'Without one yet', value: '63.7%' },
  ...
  admin: {
    ...
    overview: {
      ...
      body: 'Live counts from the database, and both accuracy figures the model actually reports — never just the flattering one.',
      stats: {
        totalAlumni: {
          label: 'Total alumni',
        ...
        pendingContributions: {
          ...
          caption: 'Alumni submissions via /contribute, awaiting review before they count.',
          zeroHint: 'Nothing waiting on review. Submissions through /contribute will appear here the moment they’re sent, and stay until a person approves or rejects them.',
        },
      },
      chart: {
        ...
        emptyBody: 'This chart fills in once alumni_profiles has verified rows — the seeded 2025 survey data, or contributions a reviewer has approved.',
        loading: 'Loading chart…',
      },
      model: {
        kicker: 'model accuracy',
        title: 'Both figures. Not just the good one.',
        body: 'Top-3 accuracy — whether a student’s actual field ended up in their top three matches — measured across all verified alumni. Shown here with equal weight on purpose: quoting only the higher number is how a demo becomes a claim nobody can stand behind.',
        version: 'Model version',
        withRoute: 'With a stated pre-U route',
        withoutRoute: 'Marginalised — no route yet',
        baseline: 'Naive baseline — most popular field',
        n: 'n',
      },
    },
    users: {
    ...
    responses: {
      ...
      filterDisagreements: 'Disagreements',
      filterDisagreementsHint: 'Submissions where the same student later contributed a different field than the model’s top pick — the clearest signal for improving the model.',
      scanLimitedNote: 'This view inspected the most recent {limit} submissions. Older ones were not checked — “disagreement” is matched in code rather than by a database query, so it cannot be filtered across the whole table.',
      columns: {
        submitted: 'Submitted',
        student: 'Student',
      ...
      emptyDisagreementsNoContributionsTitle: 'No contributions to compare against yet.',
      emptyDisagreementsNoContributionsBody: 'This filter matches a submission’s original answers against alumni who later re-submitted the same answers through /contribute with a different field of study. No one has contributed yet, so there is nothing to compare — the moment a contribution matches a past submission with a different outcome, it will appear here.',
      emptyDisagreementsNoneTitle: 'No disagreements found.',
      emptyDisagreementsNoneBody: 'Every matched contribution so far agrees with the model’s top pick for that submission.',
      detail: {
        ...
        disagreementTitle: 'This student later disagreed with the model.',
        disagreementBody: 'A contribution matching these original answers reported studying a different field than the model’s top pick.',
        disagreementModelSaid: 'Model’s top pick',
        disagreementTheyContributed: 'They later contributed',
        marginalisedNotice: 'This result was marginalised — the student had not settled on a pre-U route yet.',
      },
    },
    contributions: {
      metaTitle: 'Contributions — Admin — ARAH',
      kicker: 'admin · contributions',
      title: 'The moderation queue.',
      body: 'Alumni submissions via /contribute, awaiting review. Approving admits a row to field_stats and the next retrain; rejecting sets it aside without deleting it — either can be undone later by a database edit, never by this screen.',
      refreshGateNotice:
        'Public figures on /explore only move once at least 3 new verified rows have landed for a field since they were last published. Approving one contribution alone may not visibly change anything there — that’s expected, not a bug.',
      emptyTitle: 'Nothing waiting on review.',
      emptyBody: 'Submissions through /contribute will appear here the moment they’re sent, and stay until you approve or reject them.',
      submittedLabel: 'Submitted',
      fieldLabel: 'Field of study',
      satisfactionLabel: 'Satisfaction',
      adviceLabel: 'Advice',
      reasonsLabel: 'Reasons for choosing',
      answersKicker: 'the ten predictive answers',
      consequence: {
        crosses:
          'Approving this makes {field} n={after}, which lifts it above the suppression threshold (currently suppressed below 10) — {field} would start showing real numbers on /explore instead of “too few students yet”.',
        staysSuppressed:
          'Approving this makes {field} n={after} — still below the suppression threshold (10), so /explore keeps showing it as too few students.',
        staysShown:
          'Approving this makes {field} n={after}. It already clears the suppression threshold, so this changes the count but not whether it’s shown.',
        firstPublish: 'This would be {field}’s first published aggregate.',
        needsMore: 'and {n} more approval{plural} for {field} before the public figures actually move.',
        refreshesNow: 'and would refresh {field}’s public figures immediately — enough have landed since the last publish.',
      },
      approve: 'Approve',
      reject: 'Reject',
      approving: 'Approving…',
      rejecting: 'Rejecting…',
      confirmRejectTitle: 'Reject this contribution?',
      confirmRejectBody: 'It won’t be deleted — the row stays in the database, just out of this queue. A mistake here is recoverable.',
      confirmRejectConfirm: 'Yes, reject it',
      confirmRejectCancel: 'Cancel',
      approvedToast: 'Approved. It now counts toward field_stats and the next retrain.',
      rejectedToast: 'Rejected — set aside, not deleted.',
      errorToast: 'That didn’t go through. Refresh and try again.',
      loadErrorTitle: 'Couldn’t load the moderation queue just now.',
      loadErrorBody: 'Refresh to retry.',
    },
    algorithmTester: {
