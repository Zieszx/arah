...
export async function getSurveyPage({
  ...
  // A search can shrink the result set under an admin sitting on page 7, and
  // PostgREST answers an out-of-range range with an ERROR (PGRST103) and a
  // null count rather than an empty page — so the clamp has to fetch the
  // total separately. runPagedQuery owns that; see its header for the bug.
  return runPagedQuery({
    label: 'admin survey-data',
    page,
    pageSize,
    build: () => {
      ...
      return applyOrder(builder, sort, order);
    },
    count: async () => {
      ...
      if (error) {
        console.error('admin survey-data: count failed:', error.code ?? error.message);
        return 0;
      }
      return total ?? 0;
    },
