...
import { pageCount, clampPage } from '@/lib/admin/pagination';
...
export async function getResponsesList({ page = 1, pageSize = 25, filter = 'all' } = {}) {
  ...
  try {
    ...
    // its header. The disagreements view asks for a fixed window from 0, so
    // it is never out of range and takes the same path harmlessly.
    const paged = await runPagedQuery({
      label: 'admin responses',
      page: disagreementsOnly ? 1 : page,
      pageSize: disagreementsOnly ? DISAGREEMENT_SCAN_LIMIT : pageSize,
      build: () =>
        supabase
          .from('quiz_responses')
