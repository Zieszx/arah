 ...
 * treated as "not answered". Mirrors the set of inputs for which Python's
 * `float(raw)` raises (None, "", whitespace-only strings, non-numeric
 * strings, and — crucially — lists/arrays, since `float([5])` raises
 * TypeError even though JS's `Number([5])` silently coerces via
 * `toString()`) so both sides fall back to the same midpoint default.
 */
function toFiniteOrNaN(rawVal) {
  const isBlank =
    rawVal === null ||
    rawVal === undefined ||
    Array.isArray(rawVal) ||
    (typeof rawVal === 'string' && rawVal.trim() === '');
  if (isBlank) return NaN;
...
export function encodeAnswers(answers = {}) {
  ...
  for (const g of spec.groups) {
    if (g.type === 'num') {
      const raw = toFiniteOrNaN(answers[g.key]);
