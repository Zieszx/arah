...
const SECTION_BY_KEY = {
  ...
  stream: 'about',
  preu: 'heading',
  // Not real quiz keys — the quiz's own ten questions never include these
  // (orderedGroups() only ever iterates getGroups(), the feature_spec.json
  // groups). /contribute (Plan 4, Task 4) reuses QuestionCard/OptionGrid
  // for its three outcome questions and calls sectionForGroup() on the
  // same synthetic group shape lib/contribute/submission.js#outcomeGroups
  // returns, so they need a section here too.
  field_of_study: 'outcome',
  reasons: 'outcome',
