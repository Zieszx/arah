 ...
 * as null/NaN. `alumni_count` and `confidence` are always present.
 */
export function annotateRanked(ranked, statsRows) {
  ...
  return ranked.map(({ field, probability }) => {
    ...
    const count = stats ? (finiteNumberOrNull(stats.sample_size) ?? 0) : 0;
    const entry = {
      field,
      probability,
      alumni_count: count,
      confidence: confidenceTier(count),
      // A field with no stats row has nothing reportable — treat it as
      // suppressed rather than inventing zeros.
      suppressed: stats ? stats.suppressed !== false : true,
    };
